...
const PANES: { id: SettingsPane; label: string; glyph: IconName; desc: string }[] = [
  {
    ...
    label: "智能体",
    glyph: "Agent",
    desc: "系统内置代理与自定义代理的能力边界、模型与工具集。",
  },
  {
    id: "connect",
    label: "连接层",
    glyph: "Plug",
    desc: "受控连接统一管理外部调用，支持 MCP 与自建系统接入。",
  },
  {
    id: "env",
    label: "环境配置",
...
function AgentsPane({ onToast }: { onToast: Toast }) {
  ...
  const toggle = (id: string) => {
    ...
    onToast({
      ...
      body: `${a?.name} ${a?.enabled ? "不再参与任务调度" : "已加入可调度池"}`,
    });
  };

  const create = () => {
    const name = draftName.trim() || "未命名代理";
    const id = `a-${Date.now()}`;
    setAgents((prev) => [
      ...prev,
      {
        ...
        glyph: "Sparkle",
        tint: "plum",
        tagline: draftPrompt.trim().slice(0, 60) || "自定义代理，尚未填写职责描述。",
    ...
    onToast({ tone: "ok", title: "已创建自定义代理", body: name });
  };

  return (
    <div className="split">
      <div className="split__list">
        <SectionLabel text="系统内置" hint={`${system.length} 类`} />
      ...
      <aside className="split__detail">
        <div className="detail__head">
          ...
          <div>
            ...
            <span className="detail__kind">
              {active.kind === "system" ? "系统内置" : "自定义"}
            </span>
          </div>
        </div>
        <p className="detail__desc">{active.tagline}</p>
        ...
        <dl className="kv">
          ...
          <div>
            <dt>调用次数</dt>
            <dd className="mono">{active.runs.toLocaleString()}</dd>
          </div>
          <div>
            <dt>成功率</dt>
            <dd className="mono">{active.success ? `${active.success}%` : "—"}</dd>
          </div>
        </dl>
        ...
        <div className="tagPick tagPick--static">
          {active.tools.map((t) => (
            <span key={t} className="tag" data-on="true">
              <span className="mono">{t}</span>
            </span>
          ))}
        </div>
        ...
        <div className="detail__foot">
          <button
            ...
            onClick={() =>
              onToast({ tone: "ok", title: "已进入编辑", body: `${active.name} · 演示动作` })
            }
          >
          ...
          <button
            ...
            onClick={() =>
              onToast({ tone: "ok", title: "已设为默认", body: `新任务将由「${active.name}」承接` })
            }
          >
            设为默认
          </button>
  ...
  return (
    <div className="stack">
      <div className="statRow">
        ...
        <Stat label="24h 外部调用" value={calls.toLocaleString()} hint="全部经网关代理" />
        <Stat label="拦截" value="37" hint="未在白名单内" tone="warn" />
      </div>

      <div className="barRow">
        <div className="segment">
          {(["all", "mcp", "internal", "saas"] as const).map((k) => (
      ...
      {adding && (
        <form
          ...
          onSubmit={(e) => {
            ...
            setDraftName("");
            setDraftEndpoint("");
            onToast({ tone: "ok", title: "连接已登记", body: `${name} · 默认需审批放行` });
          }}
        >
          <div className="form__row">
            <label>类型</label>
            <div className="segment">
              {(["mcp", "internal", "saas"] as const).map((k) => (
        ...
        {shown.map((c, i) => (
          <article className="conn" key={c.id} style={{ ["--i" as string]: i }}>
            <span className="conn__glyph" data-kind={c.kind}>
              {c.kind === "mcp" ? <Icon.Plug size={16} /> : c.kind === "internal" ? <Icon.Cube size={16} /> : <Icon.Globe size={16} />}
            </span>
            ...
            <div className="conn__guard">
              <span className="conn__k">管控</span>
              <span className="conn__badge" data-guard={c.guard}>
                <Icon.Shield size={12} />
                {guardLabel[c.guard]}
              </span>
              <div className="meter">
                <div className="meter__fill" style={{ width: `${c.quota}%` }} />
              </div>
            </div>
            ...
            <div className="conn__act">
              <button
                ...
                aria-label="配置"
                onClick={() => onToast({ tone: "ok", title: "打开配置", body: `${c.name} · 演示动作` })}
              >
                <Icon.Sliders size={14} />
              </button>
              <button
                ...
                onClick={() => {
                  setList((prev) =>
                    prev.map((x) =>
                      x.id === c.id ? { ...x, state: x.state === "paused" ? "linked" : "paused" } : x,
                    ),
                  );
                  onToast({
      ...
      {tab === "cloud" ? (
        <>
          ...
          <div className="varList">
            {envVars.map((v) => (
              <div className="var" key={v.key}>
                ...
                <button
                  className="iconBtn iconBtn--sm"
                  aria-label="删除"
                  onClick={() => onToast({ tone: "warn", title: "演示动作", body: `未真正删除 ${v.key}` })}
                >
                  <Icon.Trash size={13} />
                </button>
              </div>
            ...
            <button
              className="var var--new"
              onClick={() => onToast({ tone: "ok", title: "新增变量", body: "演示动作" })}
            >
              <Icon.Plus size={14} />
              添加变量
            </button>
        ...
        <>
          ...
          <SectionLabel text="资源上限" hint="超出即中断并回收" />
          ...
          <div className="noteCard">
            <Icon.Shield size={16} />
            <p>
              沙箱内所有出网请求都会回到<strong>连接层</strong>做白名单校验；关闭网络访问后，代理仅能读写工作区与执行本地命令。
            </p>
          </div>
        </>
      )}
