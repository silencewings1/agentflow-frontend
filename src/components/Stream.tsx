...
/* ================= 节点总结（上游 + 任务项明细 + 下游） ==================
   ...
   任务项以 DAG 链呈现：上游 → 任务节点逐条 → 下游，节点之间用流向箭头
   连接；任务节点的状态色（done=sage / active=accent / todo=弱化）让
   「完成度」在整条链上一眼可读。 */
...
function NodeSummary({ e, style }: { e: Extract<AgentEvent, { kind: "node-summary" }>; style: object }) {
  ...
  const lastTaskStatus = e.tasks.length ? e.tasks[e.tasks.length - 1].status : "todo";
