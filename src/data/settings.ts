...
export const skills: Skill[] = [
  { id: "sk-browseruse", name: "浏览器自动化", namespace: "browseruse", desc: "驱动浏览器执行导航、点击、填表、截图等操作，支持多步页面交互与元素验证。", source: "builtin", enabled: true, version: "1.0.3", triggers: "打开网页、浏览 URL、页面交互" },
  { id: "sk-dynamic-ui", name: "内联可视化", namespace: "dynamic-ui", desc: "在对话中内联渲染 SVG 图表、流程图与交互式组件，辅助理解。", source: "builtin", enabled: true, version: "1.0.0", triggers: "画图、可视化、图表、流程图" },
  { id: "sk-skill-creator", name: "技能创建器", namespace: "skill-creator", desc: "创建自定义 Skill，封装可复用的 API 操作或多步流程。", source: "builtin", enabled: false, version: "1.0.0", triggers: "创建技能、封装 Skill" },
  { id: "sk-code-review", name: "代码审查", namespace: "dev:code-review", desc: "以独立视角审查代码质量、安全风险与坏味道，输出结构化问题清单与修复建议。", source: "dev", enabled: true, version: "1.0.0", triggers: "审查代码、review、安全风险" },
  { id: "sk-static-analysis", name: "静态分析", namespace: "dev:static-analysis", desc: "对源码执行 AST 级静态分析，检测未捕获异常、空指针与资源泄漏。", source: "dev", enabled: true, version: "1.0.0", triggers: "静态分析、lint、代码质量" },
  { id: "sk-deps-audit", name: "依赖审计", namespace: "dev:deps-audit", desc: "解析依赖树，标注已知漏洞（CVE）与过时版本，给出升级或固定建议。", source: "dev", enabled: true, version: "1.0.0", triggers: "依赖漏洞、CVE、升级建议" },
  { id: "sk-refactor", name: "重构建议", namespace: "dev:refactor", desc: "识别长方法、重复代码与复杂条件分支，生成最小重构方案与 diff。", source: "dev", enabled: true, version: "1.0.0", triggers: "重构、坏味道、重复代码" },
  { id: "sk-codegen", name: "代码生成", namespace: "dev:codegen", desc: "根据接口定义或类型声明生成样板代码：DTO、Builder、Mapper 等。", source: "dev", enabled: true, version: "1.0.0", triggers: "生成代码、DTO、样板代码" },
  { id: "sk-api-mock", name: "API Mock", namespace: "dev:api-mock", desc: "根据 OpenAPI / GraphQL Schema 生成 Mock Server 与请求/响应样例。", source: "dev", enabled: false, version: "1.0.0", triggers: "mock server、接口模拟" },
  { id: "sk-reverse-eng", name: "逆向工程", namespace: "dev:reverse-eng", desc: "从二进制或无文档代码推断模块边界与调用关系，生成架构草图。", source: "dev", enabled: false, version: "1.0.0", triggers: "逆向、无文档代码、架构推断" },
  { id: "sk-perf-profile", name: "性能分析", namespace: "dev:perf-profile", desc: "采集 CPU/内存/火焰图数据，定位热点函数与内存分配瓶颈。", source: "dev", enabled: false, version: "1.0.0", triggers: "性能分析、火焰图、瓶颈" },
  { id: "sk-unit-test", name: "单元测试生成", namespace: "test:unit-gen", desc: "为改动函数自动生成 JUnit / Jest / pytest 用例，覆盖正常、边界与异常分支。", source: "test", enabled: true, version: "1.0.0", triggers: "单元测试、JUnit、pytest" },
  { id: "sk-integration-test", name: "集成测试生成", namespace: "test:integration-gen", desc: "根据模块间接口契约生成集成测试用例，验证跨模块调用链与数据流。", source: "test", enabled: true, version: "1.0.0", triggers: "集成测试、接口契约、跨模块" },
  { id: "sk-e2e-test", name: "端到端测试", namespace: "test:e2e", desc: "通过浏览器自动化编排用户操作流程，验证从入口到落库的完整链路。", source: "test", enabled: false, version: "1.0.0", triggers: "E2E、端到端、用户流程" },
  { id: "sk-coverage", name: "覆盖率分析", namespace: "test:coverage", desc: "采集行/分支/方法覆盖率，标注未覆盖路径并推荐补充用例优先级。", source: "test", enabled: true, version: "1.0.0", triggers: "覆盖率、未覆盖路径" },
  { id: "sk-benchmark", name: "性能基准测试", namespace: "test:benchmark", desc: "生成 JMH / Benchmark.js 基准测试，对比改动前后的吞吐与延迟。", source: "test", enabled: false, version: "1.0.0", triggers: "基准测试、JMH、吞吐" },
  { id: "sk-security-test", name: "安全测试", namespace: "test:security", desc: "执行注入、XSS、越权与依赖漏洞扫描，输出可复现的攻击向量清单。", source: "test", enabled: false, version: "1.0.0", triggers: "安全测试、注入、XSS" },
  { id: "sk-mutation-test", name: "变异测试", namespace: "test:mutation", desc: "对源码注入变异体，检测现有测试能否捕获，量化测试有效性。", source: "test", enabled: false, version: "1.0.0", triggers: "变异测试、测试有效性" },
  { id: "sk-doc-gen", name: "API 文档生成", namespace: "doc:api-doc", desc: "从代码注解或接口定义生成 OpenAPI / GraphQL 文档与变更日志。", source: "doc", enabled: true, version: "1.0.0", triggers: "API 文档、OpenAPI、变更日志" },
  { id: "sk-doc-writing", name: "文档写作", namespace: "doc:writing-guide", desc: "PRD、产品需求、技术提案、研究报告等结构化文档写作。", source: "doc", enabled: true, version: "1.0.0", triggers: "写文档、PRD、报告" },
  { id: "sk-html-report", name: "HTML 报告", namespace: "doc:html-report", desc: "生成自包含的 HTML 交付物：研究报告、白皮书、仪表盘等。", source: "doc", enabled: false, version: "1.0.0", triggers: "HTML 报告、白皮书" },
  { id: "sk-frontend-design", name: "前端设计", namespace: "frontend:design", desc: "创建高设计质量的前端界面，避免通用 AI 风格，产出生产级组件与页面。", source: "frontend", enabled: true, version: "1.0.0", triggers: "构建页面、UI 组件、前端应用" },
];
