...
- 阶段 1.6 起，前端是 AF API 客户端。任务、工作流、节点、门禁、交付物和轨迹必须通过 `src/api/` 的客户端读取或操作；`src/data/*.ts` 只保留为脱敏 fixture 与 `demo-deterministic` 兜底，不得作为“已执行/已通过/已推送”的事实源。
- 本地没有配置 `VITE_AF_API_BASE_URL` 时，`src/api/client.ts` 使用显式标记的 fixture adapter，保证设计演示可启动；配置该变量后切换到真实 AF API，不需要改组件状态模型。
- 定位是**真实控制面的可交互载体**：它要把「AI 参与研发」从「单点辅助」推进到「可信协同」这件事，用界面讲清楚。
...
- **状态只放在 `App.tsx`**。子组件不持有跨组件状态；纯本地的 UI 开关（展开/折叠、局部筛选）可以自己持有。服务端状态通过 `src/api/` 注入，不在组件内直接调用 `fetch`。
