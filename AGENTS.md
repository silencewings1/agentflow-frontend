...
│  ├─ TopBar.tsx           面包屑 + 门禁轨 + 模型/审批模式/开 PR
...
- **界面不允许暗示一个不成立的推进**。当存在未闭环的必需证据时，「开 PR」按钮通过 `data-blocked="true"` 变为 `--gold` 受阻态。这条规则可以推广：**任何会造成不可逆后果的操作，在前置条件未满足时必须显式呈现受阻，而不是照常可点然后报错。**
